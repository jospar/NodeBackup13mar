...
int connectTCP(char * buff)
{
   ...
   printf("Port is %d jagh%d",port,port+1);
...
//    enqueue(udpClientIncQueue,(void *)buffer);   
      ...
      if(loop == 0)
      {
         ...
         int ne = 0;
         if(( ne = pthread_create(&sctp_client_sender, NULL, sendOverSctp,tokenMsg)))
            printf("Error creating pthread sctp sender %d\n", ne);
         else
            printf("sctp_client_sender thread created socket %d \n",sctp_sock);


         int ne1 = 0;
         if(( ne1 = pthread_create(&sctp_client_receiver, NULL, recvOverSctp,&sctp_sock)))
         {
            printf("Error creating pthread sctp_client_receiver %d\n", ne1);
         }
         else
            printf("sctp_client_receiver thread created socket %d \n",sctp_sock);
